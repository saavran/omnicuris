...
class QuestionOne extends Component {
    ...
    submitAnswer = () => {
        ...
        localStorage.setItem('questionOne', JSON.stringify(result))
    }
    showError = () => {
        ...
        return (
            <div>
                <Row>
                    <Col span={4}></Col>
                    <Col span={16}>
                        {this.props.quiz.slice(0, 1).map(question => {
                            return <h1 style={{ marginTop: 200 }}>{question.question}</h1>
                        })}
                        <RadioGroup onChange={this.onChange} value={this.state.value}>
                            {this.props.quiz.slice(0, 1).map(option => {
                        ...
                        {this.state.value === 'none' ? (<Button onClick={this.showError} type="primary">Next Question</Button>) : (<Link to="/q2"><Button onClick={this.submitAnswer} type="primary">Next Question</Button></Link>)}
                    </Col>
